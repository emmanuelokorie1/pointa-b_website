...
export default function ErrorPage({ error, reset }: ErrorProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100000] flex flex-col items-center justify-center bg-[#3B007A] overflow-y-auto px-6 py-12">
      {/* Main Container */}
      <div className="relative w-full max-w-lg bg-[#1a1028] border border-white/20 rounded-none p-8 md:p-10 flex flex-col items-center text-center z-10">
        
        {/* Caution Box Logo */}
        <div className="relative w-24 h-24 mb-6 flex items-center justify-center">
          <div className="absolute w-20 h-20 rounded-none bg-red-500/10 border border-red-500/30 flex items-center justify-center">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full justify-center">
          ...
          <button
            onClick={() => reset()}
            className="w-full sm:w-auto bg-[#D6FF38] hover:bg-[#c6ef20] text-[#3B007A] text-sm font-bold px-8 py-3.5 rounded-none"
          >
            Try Again
          </button>
          ...
          <button
            ...
            className="w-full sm:w-auto bg-transparent hover:bg-white/10 text-white text-sm font-bold px-8 py-3.5 rounded-none border border-white/20"
          >
            Go Home
          </button>
        </div>
        ...
        <details className="w-full mt-8 text-left group">
          <summary className="text-white/40 hover:text-white/70 text-xs font-bold uppercase tracking-wider cursor-pointer list-none flex items-center justify-center gap-1.5 select-none">
            <span>Show Diagnostic Details</span>
            <svg className="w-3.5 h-3.5 stroke-current fill-none stroke-2" viewBox="0 0 24 24">
              <polyline points="6 9 12 15 18 9" />
            </svg>
          </summary>
          <div className="mt-4 p-4 rounded-none bg-black border border-white/20 text-xs text-red-300 font-mono overflow-auto max-h-40 leading-relaxed">
