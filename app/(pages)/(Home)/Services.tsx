...
const ServiceCard = ({
    ...
    return (
        <div
            ref={cardRef}
            className={`relative w-full h-[400px] xs:h-[450px] sm:h-[500px] md:h-[540px] lg:h-[600px] rounded-[1.25rem] md:rounded-none overflow-hidden cursor-pointer transition-transform duration-500 ease-out select-none fade-in-up ${cardVisible ? 'is-visible' : ''} z-10`}
            style={{
                clipPath: isMobile ? 'none' : card.clipPathId,
                transform: isMobile ? 'scale(1)' : card.defaultTransform,
        ...
        >
            <div className="absolute inset-0 w-full h-full overflow-hidden">
                {card.slides.map((slide, i) => (
                    <Image
                        key={i}
                        src={slide}
                        alt={`Delivery service showcase ${index + 1} slide ${i + 1}`}
                        fill
                        className={`object-cover transition-opacity duration-1000 ease-in-out ${i === imageIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-[#270B4B]/60 via-transparent to-transparent z-20 pointer-events-none" />
            <div className="absolute inset-0 border-2 border-transparent z-30 pointer-events-none rounded-[1.25rem] md:rounded-none" />
        </div>
    );
};
    ...
    return (
        ...
        >
            <div className="max-w-[90%] lg:max-w-[85%] xl:max-w-[85%] mx-auto relative z-10">
                ...
                >
                    <div className="bg-white/90 border border-[#8E24FF]/20 text-[#8E24FF] text-[11px] sm:text-xs font-bold px-6 py-2.5 rounded-full uppercase tracking-[0.15em] inline-flex items-center gap-2.5 mb-6">
                        Premium Delivery Services
                    </div>

                    <h2 className="text-[#0B0F19] text-[2.5rem] sm:text-[3.5rem] md:text-[4.2rem] font-bold tracking-tight font-sans leading-[1.1] max-w-4xl mb-6">
