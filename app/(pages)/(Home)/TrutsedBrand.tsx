            ...
            <div className="relative w-full overflow-hidden py-4 z-10">
                ...
                <div className="flex items-center gap-10 sm:gap-14 w-max animate-marquee-horizontal hover:[animation-play-state:paused] cursor-pointer py-2">
                    {brands.map((brand, index) => (
                        <div 
                            key={index} 
                            className="flex items-center justify-center px-10 py-6 min-w-[200px] sm:min-w-[240px] rounded-2xl bg-white/95 border border-white/60 shadow-[0_8px_32px_rgba(0,0,0,0.02),inset_0_1px_0_rgba(255,255,255,0.6)] group hover:bg-white hover:border-primary/15 hover:shadow-[0_12px_40px_hsla(var(--primary)/0.06)] transition-all duration-500 relative overflow-hidden"
                        >
                            ...
                            {/* Brand Logo Wrapper */}
                            <div className="relative w-36 h-9 sm:w-40 sm:h-10 opacity-70 group-hover:opacity-100 transition-all duration-500 transform group-hover:scale-105 flex items-center justify-center" style={{ position: 'relative' }}>
                                <Image
                                    src={brand.logo}
                                    alt={`${brand.name} logo`}
                                    fill
